<template>
  <div class='home-category' @mouseleave=" categoryId=null">
    <ul class="menu">
      <li :class="{active:categoryId===item.id}" v-for="item in numlist" :key="item.id" @mouseover="categoryId=item.id">
        <RouterLink :to="`/category/${item.id}`">{{item.name}}</RouterLink>
        <template v-if="item.children">
          <RouterLink v-for="sub in item.children" :key="sub.id" :to="`/category/sub/${sub.id}`">{{sub.name}}</RouterLink>
        </template>
        <template v-else>
          <XtxSkeleton width="40px" height="18px" style="margin-right:5px" bg="rgba(255,255,255,0.2)"/>
          <XtxSkeleton width="40px" height="18px" style="margin-right:5px" bg="rgba(255,255,255,0.2)"/>
        </template>
      </li>
    </ul>
      ...
      <ul v-if=" currCategory && currCategory.goods ">
        <li v-for="item in currCategory.goods" :key="item.id">
          <RouterLink to="/">
            ...
            <div class="info">
              <p class="name ellipsis-2">{{item.item}}</p>
      ...
      <ul  v-if=" currCategory && currCategory.brands ">
        <li class="brand" v-for="brand in currCategory.brands" :key="brand.id">
          <RouterLink to="/">
            ...
            <div class="info">
              <p class="place"><i class="iconfont icon-dingwei"></i>{{}}</p>
...
export default {
  ...
  setup () {
    ...
    const currCategory = computed(() => {
      return numlist.value.find(item => item.id === categoryId.value)
    })
    // 获取品牌推荐数据

    ...
    <div class="container">
      ...
      <div class="cart">
        <table>
          ...
          <!-- 有效商品 -->
          <tbody>
            <tr v-if="$store.getters['cart/vaildList'].length===0">
              <td colspan="6">
                <CartNone/>
              </td>
            </tr>
            <tr v-for="goods in $store.getters['cart/vaildList']" :key="goods.skuId">
              ...
              <td class="tc">
                ...
                <p v-if="goods.nowPrice-goods.price>0">比加入时降价 <span class="red">&yen;{{goods.nowPrice-goods.price}}</span></p>
              </td>
              <td class="tc">
                <XtxNumbox @change="$event=>updateCount(goods.skuId,$event)" :modelValue="goods.count" />
              </td>
          ...
          <!-- 无效商品 -->
          <tbody v-if="$store.getters['cart/invaildList']">
            <tr><td colspan="6"><h3 class="tit">失效商品</h3></td></tr>
            <tr v-for="goods in $store.getters['cart/invaildList']" :key="goods.skuId">
      ...
      <div class="action">
        ...
        <div class="total">
          共 {{$store.getters['cart/vaildTotal']}} 件商品，已选择 {{$store.getters['cart/selectedTotal']}} 件，商品合计：
...
export default {
  ...
  setup () {
    ...
    const checkAll = (selected) => {
      store.dispatch('cart/allCheckCart', selected)
    }
    // 删除商品
    const deleteCart = (skuId) => {
      // 弹出消息框
      Confirm({ text: '确认要删除吗？' }).then(() => {
        // 删除商品
    ...
    const batchDeleteCart = (isClear) => {
      console.log(isClear)
      // 弹出消息框
      Confirm({ text: `确认要${isClear ? '清除' : '删除'}吗？` }).then(() => {
        // 删除商品
        store.dispatch('cart/batchDeleteCart', isClear)
        Message({ type: 'success', text: `${isClear ? '清除' : '删除'}商品成功` })
      }).catch((e) => {
      })
    }
    // 更新商品信息
    ...
    const checkOut = () => {
      ...
      if (store.getters['cart/selectedList'].length === 0) {
        return Message({ text: '请至少勾选一件商品进行结算' })
      }
      if (!profile.token) {
